...
7. **跨页共享 chrome 锁死字号与行高**：顶栏、页脚这类每页同款的组件，必须用 `var(--text-*)` 并显式写 `line-height`，不得继承 `is-readable-detail`（详见 §1.4，已由 `verify-style-tokens.mjs` 门禁强制）。
...
### 1.4 共享 chrome 必须锁死，不得继承可读模式

**这是 §1.3 的直接推论，也是已经踩过两次的坑。**

`is-readable-detail` 挂在 `<body>` 上，它做两件事：重映射低四档 `--fs-*`，并抬高 body 行高。
**凡是继承 body 的东西都会跟着变** —— 包括那些本不该变的**跨页共享 chrome**。

判据很简单：

> 这个组件在每个页面上是不是**同一个东西**？
> 是 → 它是 chrome，字号行高必须**显式锁定**。
> 否（属于页面内容）→ 让它跟随可读模式，这正是该模式的目的。

**已发生的三次事故**（都表现为「同一个组件在不同页面长得不一样」）：

| 组件 | 症状 | 根因 |
|---|---|---|
| 已移除的会话丝带 `.issue-strip` | 工作台 28px / 概览 24px | 用了 `--fs-2xs`，被重映射 |
| 顶栏 `.masthead` | 工作台 74px / 概览 67px | 导航、搜索、按钮全用 `--fs-xs`；**且行高继承 body**（28.05px vs `normal`） |
| 页脚 `.colophon` | 链接 17px / 14px | 同上，`--fs-sm` + 继承行高 |

**两条都要做，只做一条不够**：

1. 字号用**语义角色** `--text-*`，不要用会被重映射的 `--fs-2xs / --fs-xs / --fs-sm / --fs-md`
2. 在 chrome 的容器上**显式写 `line-height`** —— 顶栏那次锁完字号仍差 3px，就是行高继承漏掉了

```css
/* ✗ 错：跨页会变成两副面孔 */
.masthead-inner { /* 无 line-height，继承 body */ }
.app-tabs a     { font-size: var(--fs-xs); }

/* ✓ 对：chrome 自成一套，与页面阅读模式解耦 */
.masthead-inner { line-height: 1.25; }
.app-tabs a     { font-size: var(--text-control); }
```

**作用域要收窄。** `.pill-cap` 同时用在顶栏和正文里，只锁顶栏内的那份：

```css
.mast-actions .pill-cap { font-size: var(--text-control); }   /* 正文里的不动 */
```

新增任何共享 chrome（悬浮工具条、全局弹窗、通知条）时，先按上面的判据过一遍。
验证方法：在 `index.html`（无可读模式）与 `nav-detail.html`（有可读模式）各测一次计算值，
两边必须逐字节相同。

**共享层只能有一份。** 页面内联 `<style>` 不得用未限定的 `.masthead` / `.colophon`
等共享选择器重写基础组件。确有页面差异时，必须显式挂到页面身份或修饰状态上，
例如 `body[data-page="opening"] .masthead` 或 `.masthead.is-translucent`；不得再复制一整份共享规则。

`.issue-strip` 于 2026-08-03 退出全局 chrome：课程、学期和场次号不具备全局常驻价值，
计时与录制状态只属于教学实践播放器。页面需要这些信息时，应在当前任务的标题区、
播放控件或详情中表达，不得重建跨页黑色状态栏。
