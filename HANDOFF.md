...
- **别用 mtime 判构建新鲜度**（2026-08-04 冻结前实测）：`build.mjs --check` 原先比 mtime，
  本机全绿，但新 `git clone` 里 `exit=1` 报 23 个源文件"更新于 bundle 之后"——
  其实 clone 把所有文件时间戳刷成同一时刻，先后关系根本不存在，而两边 bundle 逐字节相同。
  已改为**内容比对**（重建到内存 vs `dist/` 已提交产物），且 check 与 build 共用同一对
  `buildConcat` / `buildApp`，两份配置不可能漂移。
  副作用是纯注释改动不再报过期——正确，因为 minify 后产物确实没变。
- **验证门禁本身要做反向测试**：上面这个修复第一次自测是往 `faq.js` 追加注释，
  `--check` 照样绿，我差点当成"修好了"。注释被 esbuild 剥掉，产物本就不变。
  换成真会进产物的语句（`window.__X__ = 1`）才复现出 exit=1。
  **改门禁后必须先造一个真失败，确认它会红。**
