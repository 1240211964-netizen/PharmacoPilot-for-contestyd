...
(function attachStation7Payload(global) {
  ...
  const payload = {
    ...
    narrative: {
      ...
      why:  "光定讲授/分析/协作比例还不够——课堂上必须预留具体时刻供教师停下来判断'学生跟上没有'。校准点位置由教师定，学情触发规则在形成性评价与适应性调控环节编辑。",
    ...
    evidenceFigure: {
      ...
      timeline: [
        ...
        { t: 12,  type: "phase",  label: "结构化实战开始 · 分析阶段（14′）" },
        { t: 26,  type: "phase",  label: "分析阶段结束 · 转入协作与质询（13′）" },
        { t: 28,  type: "anchor", anchorId: "Z2", label: "Z2 · 推演后即时投票（协作与质询阶段前段）" },
        { t: 30,  type: "phase",  label: "比较锚 · 中美欧对照" },
        { t: 39,  type: "phase",  label: "结构化实战结束（27′）· 转入反馈与小结" },
    ...
    feedbackByKey: {
      "student": {
        ...
        detail: "选择此项后，3 个 学情校准点会落入形成性评价与适应性调控环节 等待学情触发规则编辑。下游形成性评价与适应性调控环节 不允许在锚点决策规则为空时通过。",
      },
      ...
      "free": {        summary: "✕ 证据塌缩",   riskBadges: ["证据质量塌缩"],   detail: "失去评价标准约束，无法支撑后续评价证据收集。" },
    },
    ...
    horizontalLayerHooks: {
      ...
      L2: { visible: false, reason: "L2 在预期学习结果与评价证据设计环节/6/8/11 回响，学习活动与教学支架设计环节 不直接处理议程" },
