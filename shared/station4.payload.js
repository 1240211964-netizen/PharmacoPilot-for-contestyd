 ...
 * ------------------------------------------------------------
 * 教学环节 S2 目标量规 · L3 顶卡上游汇聚点（汇聚前序环节输入）。
  ...
  const payload = {
    ...
    chainTopcard: {
      ...
      outputsTo: [
        ...
        { stationId: 10, label: "表现性评价",   outKey: "目标 → 5 维量规" },
      ],
    },

    narrative: {
      what: "把教学目标改写为可观察、可评价、可由学生产出证明的学习成果；同时回应来自课程定位/2/3 的三类输入。",
      why:  "顶卡显示来自前序三个环节的关键片段，强制目标改写时同时回应定位、学情低分项、学生议程主张。本节点与评价与画像环节 的目标—评价对齐必须显性化（含量规反向修订通道）。",
    ...
    evidenceFigure: {
      ...
      goalEvidenceMap: [
        ...
        { goal: "能为每条 SWOT 配证据（量化 / 文件 / 政策）",  evidence: "小组 SWOT 产出每条配出处 · 量规第 1 维" },
        { goal: "能从 SWOT 推导 ≥ 3 条 TOWS 战略",              evidence: "30 分钟实战段产出 TOWS 表 · 量规第 4 维" },
        { goal: "能在质询环节为本组论据辩护",                 evidence: "交叉质询中每组至少应对 2 轮 · 录像评分" },
        { goal: "能指出 SWOT 工具的至少 2 个局限",            evidence: "课末口述 · 量规第 5 维「批判意识」" },
      ],
      ...
      options: [
        {
          ...
          rationale: "命中 议程贯通的第 2 个回响点。议程对目标与量规环节 的回应度仅 44%，是 4 维中最差。",
          score: 3.9,
    ...
    stateMachine: {
      ...
      D: { id: "saved",        desc: "已保存判断 · 数据流入知识与误区环节/9/10" },
