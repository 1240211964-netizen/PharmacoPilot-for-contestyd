...
(function attachStation5Payload(global) {
  ...
  const payload = {
    ...
    evidenceFigure: {
      ...
      keyMisconceptions: [
        ...
        },
        {
          ...
          intervention: "把任务表述改为「SWOT-TOWS 二步走」并把 TOWS 列为独立量规维度（占 24%）。展示阶段强制每组展示一个 SO/WT 战略矩阵。",
        },
        {
          key: "no-self-critique",
          text: "完全不指出 SWOT 工具局限",
          frequency: 0.78, stage: "lvl-6",
          intervention: "在小结段（38'-42'）专门留 4 分钟做「工具批判圆桌」，由教师抛出 2 条挑战（如「SWOT 没有时间维度」「内外边界模糊」），要求学生现场回应；写入「批判意识」量规维度（占 28%）。",
        },
      ],
    ...
    feedbackByKey: {
      "chain":          { summary: "✓ 触发所有 5 类误区",       riskBadges: ["方法论主链"],     detail: "选此后建议案例与证据环节 案例每条 SWOT 显式标注「事实 / 政策 / 数据 / 角色 / 风险边界」5 类标签，对应第 3 层证据要求。" },
    ...
    artifacts: [
      ...
      },
      {
        ...
        artifactLines: {
          ...
          action: "把 5 类误区编为「正反例对照卡」嵌入案例与证据环节 案例材料 + 课中调控环节 ZPD 锚点测温题。",
          constraints: [
            "每个误区配 1 个正例 + 1 个反例",
            "误区频率 > 50% 必须进入课中调控环节 ZPD 测温",
            "概念边界须用药事管理情境（非通用商科）",
          ],
        },
        writeback: { to: "artifactLibrary" },
    ...
    stateMachine: {
      A: { id: "locked",       desc: "未进入 · 目标与量规环节 目标判断未保存" },
      B: { id: "entered",      desc: "已进入未判断" },
      C: { id: "selected",     desc: "已选未保存" },
      D: { id: "saved",        desc: "已保存判断 · 数据流入案例与证据环节 / 7" },
