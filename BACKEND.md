...
“教学实践”页把九张卡片定义为可编辑的“课堂教学设计摘要”，点击“生成完整实践包”后接入 `/api/practice/generate`。后端只接受完整的课程、班级、课时、章节与九环节生成要求，并只在模型返回 `env01`–`env09` 九个非空字段时显示完整实践包；传入 `targetEnv` 与当前 `generatedPack` 时只重新生成一个环节。后端或模型不可用、响应结构不合格时，网页保留当前摘要和既有实践包，不做空白覆盖。完整实践包支持浏览器端导出 `.docx`、`.pdf`、`.md` 与包含四种文件的 `.zip`；导出只在教师点击按钮时运行。
